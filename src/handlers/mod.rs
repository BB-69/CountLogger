...
impl EventHandler for Handler {
    async fn ready(&self, ctx: serenity::prelude::Context, ready: serenity::model::gateway::Ready) {
        ...
        // Others
        tokio::spawn(crate::commands::relog::log_daily_counts(ctx, self.bot_data.clone()));
    }
    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        if let Some(guild_id) = msg.guild_id {
            ...
            if let Some(_count_ch_id) = guild_data.ids.counting_channel_id { if msg.content.parse::<i64>().is_ok() {
...
fn get_current_time(utc: i8) -> String {
    ...
    let offset_now = utc_now.with_timezone(&chrono::FixedOffset::east_opt(utc as i32 * 3600).unwrap());
