...
pub fn register() -> CreateCommand {
    CreateCommand::new("relog")
        .description("Refresh and update all logs from the start")
}
...
pub async fn execute(ctx: Context, command: CommandInteraction, bot_data: &BotData) {
    ...
    if let Some(guild_id) = command.guild_id {
        ...
        if let (Some(count_ch_id), Some(log_ch_id)) = (
            ...
            match get_lastmsg_day_map(&ctx.http, ChannelId::new(count_ch_id), &guild_data.settings.utc).await {
                Ok((daily_counts, last_message_id)) => {
                    ...
                    for year in years {
                        ...
                        for (part, new_log_msg) in new_log_msgs {
                            if let Some(old_id) = guild_data.ids.log_msg_map
                                .get(&year_i)
                                .and_then(|ym| ym.get(&part))
                            {
                                if log_channel.message(&ctx.http, *old_id).await.is_ok() {
                                    let _ = log_channel
                                        .edit_message(&ctx.http, *old_id, EditMessage::new().content(new_log_msg))
                                        .await;
                            ...
                            // fallback: create new
                            if let Ok(new_msg) = log_channel.send_message(&ctx.http, CreateMessage::new().content(new_log_msg)).await {
                    ...
                    log_info(&format!("🛠 Relog Done for Guild{} ({} entries)", guild_id_u64, guild_data.daily_counts.len()));
                }
                Err(e) => {
                    internal_err(&ctx, &command, &e.to_string()).await;
                }
                ...
                if let Some(log_ch_id) = guild_data.ids.log_channel_id {
                    ...
                    match fetch_new_daily_counts(
                        ...
                        Ok((new_counts, last_seen)) => {
                            ...
                            let offset = FixedOffset::east_opt(guild_data.settings.utc as i32 * 3600).unwrap();
                            let year_now = Utc::now().with_timezone(&offset).year();

                            let year_counts: BTreeMap<String, i64> = guild_data.daily_counts
                                .iter()
                            ...
                            let mut year_map = guild_data.ids.log_msg_map
                                .remove(&year_now)
                                .unwrap_or_default();

                            for (part, new_msg) in new_log_msgs.clone() {
                                if let Some(&old_id) = year_map.get(&part) {
                                    let _ = log_channel.edit_message(&ctx.http, MessageId::new(old_id), EditMessage::new().content(new_msg)).await;
                                } else if let Ok(new_msg) = log_channel.send_message(&ctx.http, CreateMessage::new().content(new_msg)).await {
                                    year_map.insert(part, new_msg.id.get());
                                }
                            }
                        ...
                        Err(e) => {
                            log_error(&format!("Failed fetching new counts for Guild{}: {}", guild_id_u64, e));
                        }
                    }
                }
            }
    ...
    loop {
        ...
        for msg in &page_msgs {
            ...
            if let Ok(num) = msg.content.parse::<i64>() {
                if let Some(offset) = FixedOffset::east_opt(utc.clone().clamp(-14, 12) as i32 * 3600) {
                    let dt: DateTime<FixedOffset> = msg.timestamp.with_timezone(&offset);
                    let key = dt.date_naive().format("%Y-%m-%d").to_string();
                    daily_counts.entry(key).and_modify(|v| *v = (*v).max(num)).or_insert(num);
                }
            }
        }
        ...
        if page_msgs.len() < 100 { break; }
        get_message = GetMessages::new().limit(100).after(page_msgs.last().unwrap().id);
    }
...
async fn get_lastmsg_day_map(
    ...
    loop {
        ...
        if new_last == last_message_id { break; }
